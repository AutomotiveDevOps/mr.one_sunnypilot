...
if arch == "Darwin":
  ...
  if qmake_path is None:
    print("Warning: qmake not found; skipping Qt toolchain configuration.")
    qt_env['QTDIR'] = ""
  else:
    qt_install_prefix = subprocess.check_output([qmake_path, '-query', 'QT_INSTALL_PREFIX'], encoding='utf8').strip()
    qt_install_headers = subprocess.check_output([qmake_path, '-query', 'QT_INSTALL_HEADERS'], encoding='utf8').strip()
    ...
    if os.path.isdir(qt_gui_path):
      qt_gui_dirs = [d for d in os.listdir(qt_gui_path) if os.path.isdir(os.path.join(qt_gui_path, d))]
      if qt_gui_dirs:
        qt_dirs += [f"{qt_install_headers}/QtGui/{qt_gui_dirs[0]}/QtGui"]
